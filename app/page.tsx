        ...
        <div className="container mx-auto px-4">
            <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-6 md:p-8 flex flex-wrap justify-center md:justify-between items-center gap-6">
                {categories.length > 0 ? categories.slice(0, 5).map((cat) => (
                    ...
                    >
                        <div className="w-14 h-14 md:w-16 md:h-16 rounded-2xl bg-gray-50 text-blue-900 flex items-center justify-center group-hover:bg-orange-500 group-hover:text-white transition-all duration-300 shadow-sm group-hover:shadow-lg group-hover:shadow-orange-500/30">
                            <Star size={28} strokeWidth={1.5} />
                        </div>
                        <span className="text-xs md:text-sm font-bold text-gray-700 group-hover:text-orange-500 uppercase tracking-wide text-center">{cat.label}</span>
                    </div>
                )) : (
      ...
      <section className="py-8 md:py-16 overflow-hidden bg-gray-50/50">
        <div className="container mx-auto px-4 relative">
          ...
          <div className="flex flex-col md:flex-row justify-between items-end mb-10 gap-4">
             <div>
                 <span className="text-orange-500 font-bold text-sm uppercase tracking-wider mb-2 block">Experiências Incríveis</span>
                 <h2 className="text-3xl md:text-4xl font-black text-blue-900">O que fazer em Foz?</h2>
                 <p className="text-gray-500 mt-2">Conheça as principais atrações da região</p>
             </div>
             <Link href="/tours/search" className="bg-orange-500 hover:bg-orange-600 text-white font-bold py-2 px-6 rounded-full transition-colors flex items-center gap-2 shadow-md shrink-0">
          ...
          <div className="relative group/carousel">
            ...
            <button
              ...
              className="hidden sm:flex absolute left-0 top-[45%] -translate-y-1/2 -translate-x-5 z-10 w-11 h-11 rounded-full bg-white border border-gray-200 shadow-lg items-center justify-center text-gray-600 hover:bg-blue-900 hover:text-white hover:border-blue-900 transition-all opacity-0 group-hover/carousel:opacity-100"
              aria-label="Anterior"
            >
              <ChevronLeft size={22} />
            </button>
